...
pub struct AppState {
    ...
    /// DB id of the synthetic ai-agent admin (#189). Set at startup when
    /// `RUNGU_API_KEY` is configured — extractors forge the agent identity
    /// with this exact id so `created_by` FKs point at a real user row.
